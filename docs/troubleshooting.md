...
**0. このアカウントは無効です と表示される**
- `config/admin_accounts` でそのスコープが `false` に設定されている
- DB管理者画面 `/db/accounts` でアカウントを有効化する
...
## DB管理者ログインが「確認中…」のままフリーズする

### 症状
PIN を入力してログインボタンを押すと「確認中…」のままボタンが戻らない。

### 原因

`SESSION_SECRET` 環境変数が Vercel に設定されていない場合、`createSession()` が例外をスローし、Server Action の Promise が reject される。このとき画面上はエラーメッセージが出ずフリーズする。

### 対処

1. Vercel ダッシュボード → Settings → Environment Variables で `SESSION_SECRET` が設定されているか確認
2. 未設定の場合は以下で生成して追加:
   ```bash
   openssl rand -base64 32
   ```
3. 設定後、Vercel を**再デプロイ**する（Settings 変更だけでは反映されない）

> 詳細は [`docs/security.md`](./security.md) の「環境変数一覧」を参照
...
1. `SESSION_SECRET` が設定されているか確認（`DB_SESSION_SECRET` ではなく `SESSION_SECRET`）
2. `DB_ADMIN_ID` / `DB_ADMIN_PW` が正しいか確認
